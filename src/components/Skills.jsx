...
const Skills = ({ className = 'skills', id = 'skills' }) => {
    return (
      <div className={`${className}`} id={`${id}`}>
        ...
        <p className={`${className}__3`} id={`${id}__3`}>I thought about architecture, biomedicine, economics, and a lot more. Nothing still felt right and I started to feel desperate. Is there anything that would fit all my strengths?</p>
        <img className={`${className}__arrow2`} id={`${id}__arrow2`} src="../src/assets/small_arrow_2.svg" alt="" />
        <p className={`${className}__4`} id={`${id}__4`}>Then I found interaction design. It connected all my interests and strengths and I immediately knew that that’s what I want to study. From that point on my dream has been <span className={`${className}__highlight`}>to become a great UI/UX designer</span>. </p>
      </div>
    );
  };
